...
import { ICommandParameter, ICommand } from "nativescript/lib/common/definitions/commands";
import { IErrors } from "nativescript/lib/common/declarations";
import * as path from "path";

export class NativeAddCommand
	implements ICommand {
	...
	protected isPlugin(projectData: IProjectData): boolean{
		return !!projectData.packageJsonData.nativescript;
	}

	

	protected getIosSourcePathBase(projectData: IProjectData) {
		if(this.isPlugin(projectData)){
			return path.join("platforms","ios","src");
		} else {
...
export class NativeAddAndroidCommand extends NativeAddSingleCommand {
  ...
  private generateJavaClassContent(packageName: string, classSimpleName: string): string {
...
`;
  ...
  private generateKotlinClassContent(packageName: string, classSimpleName: string): string {
...
`;
	...
	public doJavaKotlin(className: string, extension: string): void {
		...
		this.$logger.info(`${extension} file '${filePath}' generated successfully.`);
	  }
	  ...
	  private checkAndUpdateGradleProperties(): boolean {
		...
		if (fs.existsSync(filePath)) {
		  ...
		  if (match) {
			...
			if (useKotlin === 'false') {
				this.$errors.failWithHelp('The useKotlin property is set to false. Stopping processing.');
			  return false;
			}
	  
			if (useKotlin === 'true') {
			  this.$logger.warn('gradle.properties already contains "useKotlin=true".');
			  return true;
			}
		  } else {
		  ...
		  fs.writeFileSync(filePath, 'useKotlin=true\n');
...
export class NativeAddObjectiveCCommand extends NativeAddSingleCommand {
	...
	private doObjectiveC(className: string) {
		if(this.isPlugin(this.$projectData)){
			this.$errors.failWithHelp("Adding an Objective-C class not supported by this command in a plugin.")
			return;
		}
		const iosSourceBase = this.getIosSourcePathBase(this.$projectData);
	...
	private generateOrUpdateModuleMap(headerFileName: string, moduleMapPath: string): void {
	...
	private generateObjectiveCFiles(className: string, classFilePath: string, interfaceFilePath: string): boolean {
...
@objc class ${className}: NSObject {
	@objc func logMessage() {
		print("Hello from ${className} class!")
	}
}`;
...
$injector.registerCommand(["native|add|objective-c"], NativeAddObjectiveCCommand);